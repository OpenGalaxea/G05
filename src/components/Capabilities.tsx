...
export function Capabilities() {
  return (
    <section className="w-full">
      ...
      <p className="text-lg md:text-xl text-neutral-300 font-light leading-[1.8] mb-8">
        We probe three axes of prompt-level control, all emerging without additional training. <strong className="text-white font-medium">Action granularity</strong>: complexity modifiers in the prompt elicit finer-grained motor behavior. <strong className="text-white font-medium">Task horizon</strong>: compositional instructions induce coherent long-horizon execution. <strong className="text-white font-medium">Out-of-distribution generalization</strong>: on the zero-shot Air Fryer and Cook Bacon tasks, we informally observe that adverbial or spatial qualifiers (e.g. "push it in hard", "vertically") tend to bring the executed motion closer to the intended sub-goal, and substituting near-synonymous verbs (e.g. <em>press</em> / <em>push in</em> / <em>close</em> the air-fryer drawer) changes which rollouts succeed. We report these as qualitative observations on small samples, leaving a systematic study to future work.
      </p>
      ...
      <p className="text-lg md:text-xl text-neutral-300 font-light leading-[1.8] mb-8">
        Enabling the native chain-of-thought lets G0.5 decompose long-horizon goals into sub-steps and ground task-relevant objects before acting — and these contributions accumulate over longer horizons: AR success goes from 1/5 to 3/5 on the zero-shot Air Fryer task and from 0/5 to 2/5 on Cook Bacon, with per-stage process score and language-following rate rising in step. Hand-scored CoT correctness stays around 80–90%, and because the reasoning lives in the same stream, the policy can re-plan closed-loop from each new observation.
      </p>
    </section>
  );
}
